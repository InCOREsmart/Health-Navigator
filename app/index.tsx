...
import { getTraditionalEvidence, searchTraditional, startEpisode, TraditionalEvidence, TraditionalPractice } from '../lib/healthNavigator';
...
export default function HomeScreen() {
  ...
  async function beginCheck() {
    setMode('check');
    setMessage('Слушаю тебя…');
    if (!connected) return;
    try { await startEpisode(text.trim() || 'Новый эпизод'); } catch { /* session may be absent; UI remains usable */ }
  }
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        {mode === 'home' && <>
          <View style={styles.coreWrap}>
            ...
            <Text style={styles.coreHint}>Индекс будет собираться из реальных данных</Text>
          </View>
          <View style={styles.metrics}>
            {metrics.map((item) => <View key={item.label} style={styles.metric}>
              <Text style={styles.metricLabel}>{item.label}</Text><Text style={styles.metricValue}>{item.value}</Text>
              <View style={styles.track}><View style={[styles.fill, { width: `${item.level}%` }]} /></View>
            </View>)}
          </View>
          <Pressable style={styles.primary} onPress={beginCheck}><Text style={styles.primaryIcon}>◉</Text><Text style={styles.primaryText}>{message}</Text></Pressable>
        ...
        {mode === 'check' && <View style={styles.panel}>
          ...
          <Text style={styles.sectionHint}>Опиши симптомы обычными словами. Система сначала проверит опасные признаки, затем задаст нужные вопросы.</Text>
          ...
          <Pressable style={styles.primary} onPress={beginCheck}><Text style={styles.primaryText}>Начать оценку</Text></Pressable>
        ...
        {mode === 'evidence' && <View style={styles.panel}>
          <Text style={styles.sectionTitle}>Знания о традиционных практиках</Text>
          ...
          {selected ? <View style={styles.evidenceBox}>
            <Pressable onPress={() => setSelected(null)}><Text style={styles.back}>← Все практики</Text></Pressable>
            <Text style={styles.cardTitle}>{selected.name}</Text><Text style={styles.meta}>{selected.system || 'Традиционная практика'} · {selected.origin_country || 'разные страны'}</Text>
            {evidence.length === 0 && !loading && <Text style={styles.body}>Для этой практики пока нет проверенной записи по конкретному показанию.</Text>}
            {evidence.map(item => <View key={item.id} style={styles.evidenceCard}>
              <Text style={styles.badge}>{item.evidence_level || 'не оценено'} · {item.certainty || 'не указано'}</Text>
              <Text style={styles.question}>{item.clinical_question}</Text>
              <Text style={styles.body}>{item.finding}</Text>
              {item.safety_notes ? <Text style={styles.warning}>Безопасность: {item.safety_notes}</Text> : null}
              {item.contraindications ? <Text style={styles.warning}>Ограничения: {item.contraindications}</Text> : null}
              {item.interactions ? <Text style={styles.warning}>Взаимодействия: {item.interactions}</Text> : null}
            </View>)}
...
function Nav({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  return <Pressable onPress={onPress} style={[styles.navItem, active && styles.navItemActive]}><Text style={[styles.navText, active && styles.navTextActive]}>{label}</Text></Pressable>;
}

const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#081014' }, scroll: { paddingHorizontal: 20, paddingTop: 58, paddingBottom: 36 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }, kicker: { color: '#7FA7A2', fontSize: 11, letterSpacing: 2.2, fontWeight: '700' }, title: { color: '#F2F7F5', fontSize: 27, fontWeight: '700', marginTop: 6 },
  status: { flexDirection: 'row', alignItems: 'center', marginTop: 4 }, dot: { width: 7, height: 7, borderRadius: 4, backgroundColor: '#72D5A3', marginRight: 7 }, statusText: { color: '#8EA5A3', fontSize: 9, letterSpacing: 1 },
  nav: { flexDirection: 'row', marginTop: 24, gap: 8 }, navItem: { paddingVertical: 9, paddingHorizontal: 13, borderRadius: 18, backgroundColor: '#10201F' }, navItemActive: { backgroundColor: '#DDF2EA' }, navText: { color: '#78908C', fontSize: 11, fontWeight: '600' }, navTextActive: { color: '#102522' },
  coreWrap: { height: 340, alignItems: 'center', justifyContent: 'center' }, orbit: { position: 'absolute', width: 260, height: 260, borderRadius: 130, borderWidth: 1, borderColor: '#29443F' }, core: { width: 178, height: 178, borderRadius: 89, backgroundColor: '#102522', borderWidth: 1, borderColor: '#467C70', alignItems: 'center', justifyContent: 'center' }, coreLabel: { color: '#8FB7B0', fontSize: 11, letterSpacing: 2 }, coreValue: { color: '#F3FBF8', fontSize: 60, fontWeight: '300', lineHeight: 68 }, coreUnit: { color: '#648681', fontSize: 9, letterSpacing: 1.5 }, coreHint: { position: 'absolute', bottom: 4, color: '#718784', fontSize: 11 },
  metrics: { gap: 10 }, metric: { flexDirection: 'row', alignItems: 'center' }, metricLabel: { color: '#829693', width: 92, fontSize: 12 }, metricValue: { color: '#D9E8E4', width: 65, fontSize: 12, fontWeight: '600' }, track: { flex: 1, height: 4, borderRadius: 2, backgroundColor: '#1A2A29', overflow: 'hidden' }, fill: { height: '100%', backgroundColor: '#67B89E', borderRadius: 2 },
  primary: { minHeight: 58, borderRadius: 29, backgroundColor: '#DDF2EA', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 24, paddingHorizontal: 20 }, primaryIcon: { color: '#102522', fontSize: 18, marginRight: 10 }, primaryText: { color: '#102522', fontSize: 16, fontWeight: '700' }, secondary: { padding: 18, alignItems: 'center' }, secondaryText: { color: '#8FB7B0', fontSize: 12 },
  panel: { marginTop: 28 }, sectionTitle: { color: '#F2F7F5', fontSize: 23, fontWeight: '700' }, sectionHint: { color: '#718784', fontSize: 12, lineHeight: 18, marginTop: 8 }, input: { minHeight: 150, marginTop: 18, padding: 16, borderRadius: 18, backgroundColor: '#10201F', color: '#E7F1EE', fontSize: 15, textAlignVertical: 'top' }, safety: { color: '#607572', textAlign: 'center', fontSize: 10, lineHeight: 15, marginTop: 12 },
  searchRow: { flexDirection: 'row', gap: 8, marginTop: 18 }, searchInput: { flex: 1, height: 48, paddingHorizontal: 14, borderRadius: 16, backgroundColor: '#10201F', color: '#E7F1EE' }, searchButton: { height: 48, paddingHorizontal: 17, borderRadius: 16, backgroundColor: '#DDF2EA', alignItems: 'center', justifyContent: 'center' }, searchButtonText: { color: '#102522', fontWeight: '700' }, practice: { marginTop: 10, padding: 16, borderRadius: 17, backgroundColor: '#10201F' }, cardTitle: { color: '#EAF4F1', fontSize: 15, fontWeight: '700' }, meta: { color: '#78908C', fontSize: 11, marginTop: 5 }, statusLine: { color: '#607572', fontSize: 10, marginTop: 9 }, evidenceBox: { marginTop: 16 }, back: { color: '#8FB7B0', fontSize: 12, marginBottom: 14 }, evidenceCard: { marginTop: 12, padding: 15, borderRadius: 17, backgroundColor: '#10201F' }, badge: { color: '#72D5A3', fontSize: 10, textTransform: 'uppercase', letterSpacing: 0.7 }, question: { color: '#DCEAE6', fontSize: 13, fontWeight: '600', marginTop: 8, lineHeight: 18 }, body: { color: '#A9BCB8', fontSize: 12, lineHeight: 18, marginTop: 8 }, warning: { color: '#C9A875', fontSize: 11, lineHeight: 17, marginTop: 9 }, error: { color: '#D89A9A', marginTop: 14, fontSize: 11 },
});
